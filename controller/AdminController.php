...
class AdminController{
	static $default_config = array(
	  ...
	  'fetch_connect_timeout' => 3, //设置fetch传输前连接的超时时间
	  'fetch_timeout' => 20,  //设置fetch的连接传输的超时时间
	  'root_path' => '?',
	  'show'=> array (
	  	'stream'=>['txt'],
	  ...
	  'is_proxy' => false,   //下载onedrive资源时是否走反代服务器
	  'proxy_server' => '',   //反代服务器地址,需要包含http or https, 只有is_proxy参数为true才有效
	);
	...
	function net(){
		if($_POST){
			config('is_proxy', empty($_POST['is_proxy'])?false:true);
			config('proxy_server', $_POST['proxy_server']);
			config('fetch_connect_timeout', intval($_POST['fetch_connect_timeout']));
			config('fetch_timeout',intval($_POST['fetch_timeout']));
		}
		$config = config('@base');
		return view::load('net')->with('config', $config);
	}
